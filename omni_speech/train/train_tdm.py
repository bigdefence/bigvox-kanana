...
class ModelArguments:
    ...
    speech_encoder: Optional[str] = field(default='openai/whisper-large-v3')
...
class LazySupervisedDataset(Dataset):
        ...
        for sample in self.list_data_dict:
            ...
            for conv in sample['conversations']:
                if isinstance(conv['value'], str):
                    conv_len += len(conv['value'].split())
                elif isinstance(conv['value'], dict):
                    # dict 형태인 경우 내부 텍스트 필드 처리
                    if 'text' in conv['value']:
                        conv_len += len(conv['value']['text'].split())
                    # 다른 필드가 있을 경우 추가 처리 가능
                else:
                    # 다른 타입인 경우 기본값 추가
                    conv_len += 1
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            for conv in sample['conversations']:
                if isinstance(conv['value'], str):
                    cur_len += len(conv['value'].split())
                elif isinstance(conv['value'], dict):
                    # dict 형태인 경우 내부 텍스트 필드 처리
                    # 필요에 따라 이 부분을 수정하여 딕셔너리 내의 텍스트 값을 추출
                    if 'text' in conv['value']:
                        cur_len += len(conv['value']['text'].split())
                    # 다른 필드가 있을 경우 추가 처리 가능
                else:
                    # 다른 타입인 경우 기본값 추가
                    cur_len += 1
...
def train(attn_implementation=None):
    ...
    if model_args.speech_encoder is not None:
        if "llama_3" in model_args.version or "kanana" in model_args.version:
    ...
    if "kanana" in model_args.version:
    ...
    elif "kanana" in model_args.version:
        tokenizer.pad_token = "<|end_of_text|>"
        tokenizer.pad_token_id = tokenizer.convert_tokens_to_ids("<|end_of_text|>")
        conversation_lib.default_conversation = conversation_lib.conv_templates["kanana"]
