        ...
        if self.sep_style == SeparatorStyle.TWO:
            ...
            for i, (role, message) in enumerate(messages):
                if message:
                    ...
                    ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
            return ret
        elif self.sep_style == SeparatorStyle.KANANA:
            wrap_sys = lambda msg: f"<|start_header_id|>system<|end_header_id|>\n\n{msg}<|eot_id|>" if len(msg) > 0 else msg
            ret = "<|begin_of_text|>" + wrap_sys(self.system)
            for i, (role, message) in enumerate(messages):
                if message:
                    ...
                    ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
            return ret
        elif self.sep_style == SeparatorStyle.QWEN_2_5:
            wrap_sys = lambda msg: f"<|im_start|>system\n{msg}<|im_end|>" if len(msg) > 0 else msg
...
conv_llama_3 = Conversation(
    system="You are a helpful language and speech assistant. " "You are able to understand the speech content that the user provides, " "and assist the user with a variety of tasks using natural language and speech.",
    roles=("user", "assistant"),
    version="llama_v3",
    messages=[],
    offset=0,
...
conv_kanana = Conversation(
    system="You are a helpful language and speech assistant. " "You are able to understand the speech content that the user provides, " "and assist the user with a variety of tasks using natural language and speech.",
    roles=("user", "assistant"),
    version="kanana",
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.KANANA,
    sep="",
    sep2="<|eot_id|>"
)
...
conv_qwen_2_5 = Conversation(
    ...
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.QWEN_2_5,
    sep="<|im_start|>",       
    sep2="<|im_end|>",        
)
